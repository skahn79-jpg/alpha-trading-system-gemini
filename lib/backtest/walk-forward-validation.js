/**
 * GATE 5N Walk-Forward / OOS 검증. 순수 CommonJS. I/O 없음.
 * Train은 메타데이터만 — 최적화 없음. 각 fold는 runSyntheticBenchmarkPipeline 실행.
 */
...
function makeError(code, extra) {
  ...
  if (extra && extra.field != null) err.field = extra.field;
  if (extra && extra.tradingDate != null) err.tradingDate = extra.tradingDate;
  if (extra && extra.foldId != null) err.foldId = extra.foldId;
  return err;
}
...
function generateWalkForwardWindows(input) {
  ...
  if (n < trainWindowSize + oosWindowSize) {
  ...
  while (true) {
    ...
    const oosStartIndex = trainEndIndex + 1;
    const oosEndIndex = oosStartIndex + oosWindowSize - 1;

    if (oosEndIndex >= n) {
      droppedIncompleteTail = oosStartIndex < n;
      break;
    }
...
function buildOosTradeIntent(oosDates, templateIntent, foldId) {
...
function buildFoldPipelineInput(input, window, pipelineBase) {
  const { tradingDates } = input;
  const { oosStart, oosEnd, oosStartIndex, oosEndIndex, foldId } = window;
  const oosDates = tradingDates.slice(oosStartIndex, oosEndIndex + 1);
  ...
  const templateIntent = Array.isArray(pipelineBase.tradeIntents) && pipelineBase.tradeIntents.length > 0
    ...
    : null;
  const tradeResult = buildOosTradeIntent(oosDates, templateIntent, foldId);
  if (!tradeResult.ok) {
...
function blockedWalkForwardResult(partial) {
  ...
  return {
    ...
    officialFolds: [],
    partialFoldResults: Array.isArray(src.partialFoldResults) ? src.partialFoldResults : (Array.isArray(src.folds) ? src.folds : []),
...
function runWalkForwardValidation(input) {
  ...
  for (const window of windowResult.windows) {
    ...
    const buildResult = buildFoldPipelineInput(input, window, input.pipelineBase);
    if (!buildResult.ok) {
      if (!firstFailureError) firstFailureError = buildResult.error;
      folds.push({
        ...foldBase,
    ...
    foldInput.benchmark = {
      market: input.market,
      benchmarkSeries: input.benchmarkSeries.slice(),
    };
    ...
    if (pipelineResult.pipelineStatus !== PIPELINE_STATUS.COMPLETED_BENCHMARK_ALPHA) {
      ...
      if (!firstFailureError) firstFailureError = rootErr;
      folds.push({
        ...foldBase,
        status: FOLD_STATUS.BLOCKED,
        totalReturn: null,
        ...
        errorCodes: uniqueCodes([
          ...(Array.isArray(pipelineResult.errorCodes) ? pipelineResult.errorCodes : []),
    ...
    const metrics = extractFoldMetrics(pipelineResult);
    folds.push({
      ...foldBase,
  ...
  for (const fold of folds) {
    if (!isFiniteNumber(fold.totalReturn) || !isFiniteNumber(fold.benchmarkReturn) || !isFiniteNumber(fold.alpha)) {
      return aggregateBlockedResult("foldMetrics");
    }
    const nextSumTotal = sumTotal + fold.totalReturn;
    if (!isFiniteNumber(nextSumTotal)) {
      return aggregateBlockedResult("meanOosTotalReturn");
    }
    sumTotal = nextSumTotal;

    const nextSumBench = sumBench + fold.benchmarkReturn;
    if (!isFiniteNumber(nextSumBench)) {
      return aggregateBlockedResult("meanOosBenchmarkReturn");
    }
    sumBench = nextSumBench;

    const nextSumAlpha = sumAlpha + fold.alpha;
    if (!isFiniteNumber(nextSumAlpha)) {
      return aggregateBlockedResult("meanOosAlpha");
    }
    sumAlpha = nextSumAlpha;
  }

  const foldCount = folds.length;
  const meanOosTotalReturn = sumTotal / foldCount;
  const meanOosBenchmarkReturn = sumBench / foldCount;
  const meanOosAlpha = sumAlpha / foldCount;

  if (!isFiniteNumber(meanOosTotalReturn) || !isFiniteNumber(meanOosBenchmarkReturn) || !isFiniteNumber(meanOosAlpha)) {
    return aggregateBlockedResult(null);
  }
