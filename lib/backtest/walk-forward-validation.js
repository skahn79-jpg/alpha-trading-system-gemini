 ...
 * 7K freeze: blockedWalkForwardResult slices folds/partialFoldResults (7G).
 *     completedWalkForwardResult shares one folds copy with officialFolds (7I).
 *     Do not unify blocked officialFolds=[] with completed shared copy.
 * 7L: foldBase copies embargoDates and postOosEmbargoDates from the window.
 * 7N freeze: pin 7L/7M foldBase embargoDates and postOosEmbargoDates copies.
 *     Do not also slice at generateWalkForwardWindows.
 * 7W freeze: leftover named-builder copies closed. generateWalkForwardWindows
 *     fail() keeps local-owned errors unsliced. Do not slice COMPLETED ledger arrays.
 * GATE 9I freeze: public walk-forward result uses walkForwardStatus. Do not unify with pipelineStatus or generic ok.
 */
...
const benchmarkDateIndex = require("./benchmark-date-index");
...
const MAX_WALK_FORWARD_TRADING_DATES = 1260;
...
function generateWalkForwardWindows(input) {
  // 7W freeze: local-owned errors. Do not slice here.
  // GATE 9B freeze: leftover-class audit after 8Z. Do not slice. Do not reopen 7W.
  ...
  if (dates.length > MAX_WALK_FORWARD_TRADING_DATES) {
...
function buildBoundedPipelineInput(pipelineBase, dataset, calendar, calendarValidation, tradeIntents, initialCapital) {
  const foldInput = {
    pipelineVersion: pipelineBase.pipelineVersion,
    calculationMode: pipelineBase.calculationMode,
    dataset,
    calendar,
    calendarValidation,
    cost: isPlainObject(pipelineBase.cost) ? deepClonePlain(pipelineBase.cost) : pipelineBase.cost,
    tradeIntents,
    initialCapital,
  };
  if (Object.hasOwn(pipelineBase, "execution")) {
    foldInput.execution = isPlainObject(pipelineBase.execution)
      ? deepClonePlain(pipelineBase.execution)
      : pipelineBase.execution;
  }
  return foldInput;
}
...
function buildFoldPipelineInput(input, window, pipelineBase, periodDates, tradeId) {
  ...
  const calendarValidation = isPlainObject(pipelineBase.calendarValidation)
    ? deepClonePlain(pipelineBase.calendarValidation)
    : {};
  calendarValidation.requiredFrom = periodStart;
  calendarValidation.requiredTo = periodEnd;
  const foldInput = buildBoundedPipelineInput(
    pipelineBase,
    sliceDatasetForOos(pipelineBase.dataset, oosDates),
    sliceCalendarForOos(pipelineBase.calendar, oosDates),
    calendarValidation,
    [tradeResult.tradeIntent],
...
function evaluateStandaloneOosFold(input, window, foldBase, oosBenchmarkIndex) {
  ...
  for (let k = 0; k < built.tiles.length; k += 1) {
    ...
    const slicedBm = benchmarkDateIndex.sliceBenchmarkFromIndex(oosBenchmarkIndex, tile);
    if (!slicedBm.ok) {
      return blockedStandaloneFold(foldBase, makeError(ERROR.OOS_FOLD_FAILED, { field: "benchmarkSeries" }));
    }
    foldInput.benchmark = {
      market: input.market,
...
// GATE 9I freeze: walk-forward family uses walkForwardStatus. Do not add generic ok.
...
function completedWalkForwardResult(partial) {
  const src = partial || {};
  // 7I: copy folds/officialFolds so callers cannot mutate the result arrays.
  const folds = Array.isArray(src.folds) ? src.folds.slice() : [];
  return {
...
function runWalkForwardValidation(input) {
  ...
  const oosBenchmarkIndex = benchmarkDateIndex.buildBenchmarkDateIndex(input.benchmarkSeries);
  ...
  for (const window of windowResult.windows) {
    ...
    const evalResult = evaluateStandaloneOosFold(input, window, foldBase, oosBenchmarkIndex);
    if (!evalResult.ok) {
