...
function blockedStandaloneFold(foldBase, error, extraCodes) {
  const codes = [ERROR.OOS_FOLD_FAILED];
...
function evaluateStandaloneOosFold(input, window, foldBase) {
  ...
  for (let k = 0; k < built.tiles.length; k += 1) {
    ...
    if (pipelineResult.pipelineStatus !== PIPELINE_STATUS.COMPLETED_BENCHMARK_ALPHA) {
      ...
      return blockedStandaloneFold(foldBase, rootErr, extraCodes);
    }
  ...
  const sumErr = assertFiniteOosTriple(sumTotal, sumBench, sumAlpha, window.foldId);
  if (sumErr) {
    return blockedStandaloneFold(foldBase, sumErr);
  }

  const meanTotal = sumTotal / n;
  const meanBench = sumBench / n;
  const meanAlpha = sumAlpha / n;
  const meanErr = assertFiniteOosTriple(meanTotal, meanBench, meanAlpha, window.foldId);
  if (meanErr) {
    return blockedStandaloneFold(foldBase, meanErr);
  }
  ...
  return {
    ...
    fold: {
      ...
      totalReturn: meanTotal,
      benchmarkReturn: meanBench,
      alpha: meanAlpha,
      errorCodes: [],
    },
  };
...
function runWalkForwardValidation(input) {
  ...
  for (const window of windowResult.windows) {
    ...
    if (!evalResult.ok) {
      if (!firstFailureError) firstFailureError = evalResult.error;
