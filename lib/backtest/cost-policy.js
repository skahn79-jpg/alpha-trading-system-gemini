...
function createCostResult(partial) {
  ...
  return {
    ...
    sellTaxes: Array.isArray(src.sellTaxes) ? src.sellTaxes.map((t) => ({ ...t })) : null,
...
function checkRatePpm(value, field, errors, extra) {
  if (!isValidRatePpm(value)) {
    errors.push(makeSafeCostError({ code: ERROR.INVALID_RATE_PPM, field, ...extra }));
    return false;
  }
  return true;
}
...
function checkRoundingMode(value, field, errors, extra) {
  if (typeof value !== "string" || !ROUNDING_MODE_SET.has(value)) {
    errors.push(makeSafeCostError({ code: ERROR.INVALID_ROUNDING_MODE, field, ...extra }));
    return false;
  }
  return true;
}
...
function validateCostPolicy(policy, context) {
  ...
  if (Object.hasOwn(policy, "policyId")) {
    if (typeof policy.policyId !== "string" || !policy.policyId.startsWith("synthetic-cost-")) {
      errors.push(makeSafeCostError({ code: ERROR.INVALID_POLICY_ID, field: "policyId", ...extra }));
    }
  }

  if (Object.hasOwn(policy, "policyVersion")) {
    if (typeof policy.policyVersion !== "string" || policy.policyVersion.length === 0) {
      errors.push(makeSafeCostError({ code: ERROR.INVALID_POLICY_VERSION, field: "policyVersion", ...extra }));
    }
  }

  if (Object.hasOwn(policy, "policyStatus")) {
    if (!POLICY_STATUS_VALUES.has(policy.policyStatus)) {
      errors.push(makeSafeCostError({ code: ERROR.INVALID_POLICY_STATUS, field: "policyStatus", ...extra }));
    }
  }

  if (Object.hasOwn(policy, "fixtureType") && policy.fixtureType !== FIXTURE_TYPE.SYNTHETIC) {
    errors.push(makeSafeCostError({ code: ERROR.UNKNOWN_FIELD, field: "fixtureType", ...extra }));
  }

  if (Object.hasOwn(policy, "notProductionData") && policy.notProductionData !== true) {
    errors.push(makeSafeCostError({ code: ERROR.INVALID_INPUT, field: "notProductionData", ...extra }));
  }

  if (Object.hasOwn(policy, "productionEligible") && policy.productionEligible !== false) {
    errors.push(makeSafeCostError({ code: ERROR.INVALID_INPUT, field: "productionEligible", ...extra }));
  }

  if (Object.hasOwn(policy, "market")) {
    if (!MARKET_VALUES.has(policy.market)) {
      errors.push(makeSafeCostError({ code: ERROR.INVALID_MARKET, field: "market", ...extra }));
    }
  }

  if (Object.hasOwn(policy, "currency") && policy.currency !== CURRENCY.KRW) {
    errors.push(makeSafeCostError({ code: ERROR.INVALID_CURRENCY, field: "currency", ...extra }));
  }

  if (Object.hasOwn(policy, "brokerChannel") && policy.brokerChannel !== BROKER_CHANNEL.SYNTHETIC_ONLINE) {
    errors.push(makeSafeCostError({ code: ERROR.INVALID_BROKER_CHANNEL, field: "brokerChannel", ...extra }));
  }
  ...
  if (effectiveFrom != null && effectiveTo != null && effectiveFrom > effectiveTo) {
    errors.push(makeSafeCostError({ code: ERROR.INVALID_EFFECTIVE_RANGE, field: "effectiveFrom", ...extra }));
  }

  if (Object.hasOwn(policy, "commission")) {
    const commission = policy.commission;
    if (!isPlainObject(commission)) {
      errors.push(makeSafeCostError({ code: ERROR.INVALID_INPUT, field: "commission", ...extra }));
    } else {
      ...
      if (Object.hasOwn(commission, "minimumBuyAmount")) {
        if (!isValidMoneyAmount(commission.minimumBuyAmount)) {
          errors.push(makeSafeCostError({ code: ERROR.INVALID_MONEY_AMOUNT, field: "commission.minimumBuyAmount", ...extra }));
        }
      }
      if (Object.hasOwn(commission, "minimumSellAmount")) {
        if (!isValidMoneyAmount(commission.minimumSellAmount)) {
          errors.push(makeSafeCostError({ code: ERROR.INVALID_MONEY_AMOUNT, field: "commission.minimumSellAmount", ...extra }));
        }
      }
  ...
  if (Object.hasOwn(policy, "sellTaxes")) {
    if (!Array.isArray(policy.sellTaxes)) {
      errors.push(makeSafeCostError({ code: ERROR.INVALID_INPUT, field: "sellTaxes", ...extra }));
    } else {
      ...
      for (let i = 0; i < policy.sellTaxes.length; i += 1) {
        ...
        if (!isPlainObject(tax)) {
          errors.push(makeSafeCostError({ code: ERROR.INVALID_INPUT, field: `sellTaxes[${i}]`, ...taxExtra }));
          continue;
        }
        ...
        if (Object.hasOwn(tax, "taxType")) {
          if (typeof tax.taxType !== "string" || tax.taxType.length === 0) {
            errors.push(makeSafeCostError({ code: ERROR.INVALID_INPUT, field: `sellTaxes[${i}].taxType`, ...taxExtra }));
...
function calculateSyntheticTradeCost(input) {
  ...
  if (!entryStatus.ok) {
    return blockedCostResult([entryStatus.error], { entryAmount, exitAmount });
  }
  const exitStatus = checkPolicyCalculationStatus(exitPolicy, input.exitTradingDate);
  if (!exitStatus.ok) {
    return blockedCostResult([exitStatus.error], { entryAmount, exitAmount });
  }
