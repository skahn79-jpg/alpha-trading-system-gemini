...
function makeError(code, extra) {
  const err = { code, severity: "ERROR" };
  if (extra && isPlainObject(extra)) {
    const allowed = ["field", "tradeId", "tradeIndex", "stage", "market", "recordIndex"];
    for (const k of allowed) {
      if (extra[k] != null) err[k] = extra[k];
    }
  }
  return err;
}
...
function processSingleTrade(intent, originalIndex, dataset, calendar, calendarValidation, cost, calculationMode) {
  ...
  // entry execution (entryDate 기준 candles[entryFound.index..])
  const entryCandles = candles.slice(entryFound.index);
  const entryExecInput = {
    ...
    candles: entryCandles.map(mapCandleForExecution),
    validationState: {
      schemaValid: true,
  ...
  const entryExecResult = executionModel.evaluateDailyBarExecution(entryExecInput);

  if (!entryExecResult.ok
    || entryExecResult.entryStatus !== executionModel.ENTRY_STATUS.FILLED
    || entryExecResult.entryPrice == null) {
  ...
  // entry date 일치 확인
  if (entryExecResult.entryTradingDate !== intent.entryDate) {
  ...
  const entryPrice = entryExecResult.entryPrice;

  // exit execution: exitDate 캔들만 포함하여 evaluateDailyBarExecution 재사용
  // exit 전용 실행: earliestExecutionTradingDate = exitDate, candles = [exitCandle]
  const exitExecInput = {
    modelVersion: (intent.modelVersion) || executionModel.MODEL_VERSION,
    calculationMode: calculationMode || "SYNTHETIC_UNIT_TEST_ONLY",
    side: executionModel.SIDE.LONG,
    ...
    entryIntent: {
      orderType: executionModel.ORDER_TYPE.MARKET_OPEN,
      signalTradingDate: entryIntentSpec.signalTradingDate,
      earliestExecutionTradingDate: intent.exitDate,
      limitPrice: null,
    },
    exitPolicy: {
      stopLossPrice: exitPolicy.stopLossPrice,
    ...
    candles: candles.slice(exitFound.index).map(mapCandleForExecution),
    validationState: {
      schemaValid: true,
      datasetVerified: false,
      backtestDataEligible: false,
  ...
  // exit 전용: entry price를 exit candle의 open으로 설정하여 sl/tp range 검사 통과
  // (exit execution에서는 exitDate 캔들의 SL/TP 로직만 필요)
  // exit를 위한 entry: 실제 entry price를 유지하되 SL/TP 검사를 우회하기 위해
  // exitCandle open을 entryPrice로 사용하는 것이 아니라, 실제 exitDate 캔들에서 SL/TP hit 확인
  // evaluateDailyBarExecution은 entryIntent.orderType=MARKET_OPEN이면 earliest 이후 첫 캔들 open으로 entry → 그게 exitCandle.open
  // 그리고 같은 캔들에서 SL/TP exit (startExitIndex = entryIndex)
  // 단, SL < exitCandle.open < TP 조건 검사가 있음. exitPolicy 가격이 이를 만족해야 함.

  const exitExecResult = executionModel.evaluateDailyBarExecution(exitExecInput);

  if (!exitExecResult.ok
    || exitExecResult.exitStatus !== executionModel.EXIT_STATUS.FILLED
    || exitExecResult.exitPrice == null) {
  ...
  // exit date 일치 확인
  if (exitExecResult.exitTradingDate !== intent.exitDate) {
  ...
  const exitPrice = exitExecResult.exitPrice;
  ...
  const costInput = {
    ...
    entryTradingDate: entryExecResult.entryTradingDate,
    exitTradingDate: exitExecResult.exitTradingDate,
    entryPrice,
    exitPrice,
  ...
  if (costResult.ok !== true) {
    ...
    for (const e of costErrors) {
      errors.push({ ...e, tradeId: intent.tradeId, tradeIndex: originalIndex });
    }
  ...
  const closedTrade = {
    ...
    quantity: intent.quantity,
    entryDate: entryExecResult.entryTradingDate,
    entryPrice,
    exitDate: exitExecResult.exitTradingDate,
    exitPrice,
...
function runSyntheticMultiTradeLifecycle(input) {
  ...
  if (!isPlainObject(input)) {
    return {
      ...
      errors: [makeError(ERROR_CODE.INVALID_INPUT, { field: null })],
    };
  }
