 ...
 * 7S: resultFail copies a non-empty errors array via slice().
 * 7T freeze: pin 7S resultFail errors slice. Do not deep clone. Do not add official flags.
 */
...
// GATE 9I freeze: resultFail has ok, status, errors. Do not add errorCodes.
function resultFail(errors) {
  // 7S: copy so callers cannot mutate the result errors array.
  const list = Array.isArray(errors) && errors.length > 0
    ? errors.slice()
    : [{ code: PERFORMANCE_STATUS.INVALID_INPUT, field: null }];
