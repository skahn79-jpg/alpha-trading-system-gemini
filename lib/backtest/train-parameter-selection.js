/**
 * GATE 5O + 5P + 5Q — Leakage-Free Train Selection / Frozen OOS Parameters.
 * 5P Train and 5Q OOS scoring: NONOVERLAPPING_THREE_BAR_TILES. Winner-only frozen OOS, no reselection.
...
function buildOosPipelineInput(pipelineBase, oosDates, periodStart, periodEnd, exitPolicy, foldId, initialCapital) {
  ...
  for (let k = 0; k < built.tiles.length; k += 1) {
    ...
    if (!tradeResult.ok) return { ok: false, error: makeError(ERROR.OOS_FOLD_FAILED, { foldId, cause: tradeResult.error.code }) };
    tradeIntents.push(tradeResult.tradeIntent);
  }
  const seen = new Set();
  for (const intent of tradeIntents) {
...
function evaluateTrainCandidate(args) {
  ...
  const build = buildTrainPipelineInput(
    pipelineBase,
    trainDates,
    trainStart,
    trainEnd,
...
function runOosWithFrozenParameters(args) {
  ...
  const build = buildOosPipelineInput(
    pipelineBase,
    oosDates,
    oosStart,
    oosEnd,
