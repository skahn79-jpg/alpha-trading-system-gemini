...
function buildPeriodPipelineInput(pipelineBase, periodDates, periodStart, periodEnd, exitPolicy, tradeId, initialCapital, failCode) {
  ...
  if (!guard.ok) {
    return { ok: false, error: makeError(code, { field: "featureWindow", tradeId }) };
  }
...
function evaluateTrainCandidate(args) {
  ...
  const tileCount = tileReturns.length;
  ...
  const trainSum = tileReturns.reduce((a, b) => a + b, 0);
  if (!isFiniteNumber(trainSum)) {
    return {
      ok: false,
      evaluation: {
        candidateId: candidate.id,
  ...
  const trainTotalReturn = trainSum / tileCount;
  if (!isFiniteNumber(trainTotalReturn)) {
    return {
      ok: false,
      evaluation: {
        candidateId: candidate.id,
...
function runOosWithFrozenParameters(args) {
  ...
  const sumTotal = tileTotals.reduce((a, b) => a + b, 0);
  const sumBench = tileBench.reduce((a, b) => a + b, 0);
  const sumAlpha = tileAlpha.reduce((a, b) => a + b, 0);
  if (
    !isFiniteNumber(sumTotal)
    || !isFiniteNumber(sumBench)
    || !isFiniteNumber(sumAlpha)
  ) {
    return {
      ok: false,
      error: makeError(ERROR.OOS_EVALUATION_NONFINITE, { foldId, field: "oosMetrics" }),
  ...
  const totalReturn = sumTotal / n;
  const benchmarkReturn = sumBench / n;
  const alpha = sumAlpha / n;
  if (
