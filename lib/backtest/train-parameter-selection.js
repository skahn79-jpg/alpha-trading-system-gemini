/**
 * GATE 5O — Leakage-Free Train Selection / Frozen OOS Parameters.
 * Train-only candidate scoring → deterministic selection → freeze → winner-only OOS.
...
const SELECTION_EVALUATION_POLICY = "FIXED_FIRST_THREE_TRADING_BARS";
const SELECTION_EVALUATION_TRADING_DAY_COUNT = 3;
...
function selectionEvaluationMeta(trainDates) {
  const hasThree = Array.isArray(trainDates) && trainDates.length >= SELECTION_EVALUATION_TRADING_DAY_COUNT;
  return {
    selectionEvaluationStart: hasThree ? trainDates[0] : null,
    selectionEvaluationEnd: hasThree ? trainDates[SELECTION_EVALUATION_TRADING_DAY_COUNT - 1] : null,
    selectionEvaluationTradingDayCount: SELECTION_EVALUATION_TRADING_DAY_COUNT,
    selectionEvaluationPolicy: SELECTION_EVALUATION_POLICY,
  };
}

function buildPeriodTradeIntent(periodDates, templateIntent, tradeId, exitPolicy) {
  if (periodDates.length < 3) {
    return {
      ok: false,
      error: makeError(ERROR.TRAIN_CANDIDATE_EVALUATION_FAILED, { field: "periodDates", tradeId }),
    };
  }
...
function buildPeriodPipelineInput(pipelineBase, periodDates, periodStart, periodEnd, exitPolicy, tradeId, initialCapital) {
  const templateIntent = Array.isArray(pipelineBase.tradeIntents) && pipelineBase.tradeIntents.length > 0
    ...
    : null;
  const tradeResult = buildPeriodTradeIntent(periodDates, templateIntent, tradeId, exitPolicy);
  if (!tradeResult.ok) {
...
function evaluateTrainCandidate(args) {
  ...
  if (pipelineResult.pipelineStatus !== PIPELINE_STATUS.COMPLETED_PERFORMANCE_METRICS) {
    const rootErr = Array.isArray(pipelineResult.errors) && pipelineResult.errors.length > 0
      ...
      : makeError(ERROR.TRAIN_CANDIDATE_EVALUATION_FAILED, { candidateId: candidate.id, foldId });
    return {
      ok: false,
      evaluation: {
        candidateId: candidate.id,
  ...
  if (!isFiniteNumber(pipelineResult.totalReturn)) {
    return {
      ...
      evaluation: {
        candidateId: candidate.id,
        trainTotalReturn: pipelineResult.totalReturn,
        status: "BLOCKED",
        stopLossPrice: candidate.stopLossPrice,
        takeProfitPrice: candidate.takeProfitPrice,
      },
...
function runOosWithFrozenParameters(args) {
  ...
  if (!build.ok) {
    return { ok: false, error: makeError(ERROR.OOS_FOLD_FAILED, { foldId, cause: build.error.code }) };
  }

  const foldInput = build.foldInput;
  foldInput.benchmark = {
    market,
    benchmarkSeries: benchmarkSeries.slice(),
  };
  ...
  if (
    ...
    return {
      ok: false,
      error: makeError(ERROR.OOS_FOLD_FAILED, { foldId, reason: "NONFINITE_OOS_METRICS" }),
    };
  }
...
function blockedSelectionResult(partial) {
  ...
  return {
    ...
    folds: Array.isArray(src.folds) ? src.folds : [],
    officialFolds: [],
    partialFoldResults: Array.isArray(src.partialFoldResults)
      ? src.partialFoldResults
      : (Array.isArray(src.folds) ? src.folds : []),
    failedStage,
...
function runWalkForwardTrainParameterSelection(input) {
  ...
  for (const fold of folds) {
    if (!isFiniteNumber(fold.totalReturn) || !isFiniteNumber(fold.benchmarkReturn) || !isFiniteNumber(fold.alpha)) {
      return aggregateBlockedResult("fold_metric");
    }
    sumTotal += fold.totalReturn;
    if (!Number.isFinite(sumTotal)) return aggregateBlockedResult("sumTotalReturn");
    sumBench += fold.benchmarkReturn;
    if (!Number.isFinite(sumBench)) return aggregateBlockedResult("sumBenchmarkReturn");
    sumAlpha += fold.alpha;
    if (!Number.isFinite(sumAlpha)) return aggregateBlockedResult("sumAlpha");
  }

  const n = folds.length;
  const meanOosTotalReturn = sumTotal / n;
  const meanOosBenchmarkReturn = sumBench / n;
  const meanOosAlpha = sumAlpha / n;
  if (!Number.isFinite(meanOosTotalReturn)) return aggregateBlockedResult("meanOosTotalReturn");
  if (!Number.isFinite(meanOosBenchmarkReturn)) return aggregateBlockedResult("meanOosBenchmarkReturn");
  if (!Number.isFinite(meanOosAlpha)) return aggregateBlockedResult("meanOosAlpha");
