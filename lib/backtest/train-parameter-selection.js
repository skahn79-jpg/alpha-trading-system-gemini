 ...
 *     assertFeatureWindowNoLookAhead with asOf = tile last date.
...
function buildPeriodPipelineInput(pipelineBase, periodDates, periodStart, periodEnd, exitPolicy, tradeId, initialCapital, failCode) {
  ...
  const asOf = periodDates[periodDates.length - 1];
  const guard = leakageGuard.assertFeatureWindowNoLookAhead({
    featureWindow: foldInput.dataset && foldInput.dataset.candles,
    featureAsOfTradingDate: asOf,
  });
  if (!guard.ok) {
...
function evaluateTrainCandidate(args) {
  ...
  const trainTotalReturn = tileReturns.reduce((a, b) => a + b, 0) / tileReturns.length;
...
function runOosWithFrozenParameters(args) {
  ...
  return {
    ok: true,
    totalReturn: tileTotals.reduce((a, b) => a + b, 0) / n,
    benchmarkReturn: tileBench.reduce((a, b) => a + b, 0) / n,
    alpha: tileAlpha.reduce((a, b) => a + b, 0) / n,
  };
}
