...
const MAX_PARAMETER_CANDIDATE_COUNT = 16;
...
function validateParameterCandidates(parameterCandidates) {
  ...
  if (parameterCandidates.length > MAX_PARAMETER_CANDIDATE_COUNT) {
    return {
      ok: false,
      error: makeError(ERROR.INVALID_PARAMETER_CANDIDATES, {
        field: "parameterCandidates",
        reason: "MAX_EXCEEDED",
      }),
    };
  }
...
function overlayCalendarValidation(pipelineBase, periodStart, periodEnd) {
...
function buildPeriodPipelineInput(pipelineBase, periodDates, periodStart, periodEnd, exitPolicy, tradeId, initialCapital, failCode) {
  ...
  const foldInput = buildBoundedPipelineInput(
    pipelineBase,
    sliceDatasetForDates(pipelineBase.dataset, periodDates),
    sliceCalendarForDates(pipelineBase.calendar, periodDates),
    overlayCalendarValidation(pipelineBase, periodStart, periodEnd),
...
function buildTrainPipelineInput(pipelineBase, trainDates, periodStart, periodEnd, exitPolicy, foldId, candidateId, initialCapital) {
  ...
  const foldInput = buildBoundedPipelineInput(
    pipelineBase,
    sliceDatasetForDates(pipelineBase.dataset, trainDates),
    sliceCalendarForDates(pipelineBase.calendar, trainDates),
    overlayCalendarValidation(pipelineBase, periodStart, periodEnd),
    tradeIntents,
    initialCapital,
  );
...
function buildOosPipelineInput(pipelineBase, oosDates, periodStart, periodEnd, exitPolicy, foldId, initialCapital) {
  ...
  const foldInput = buildBoundedPipelineInput(
    pipelineBase,
    sliceDatasetForDates(pipelineBase.dataset, oosDates),
    sliceCalendarForDates(pipelineBase.calendar, oosDates),
    overlayCalendarValidation(pipelineBase, periodStart, periodEnd),
    tradeIntents,
    initialCapital,
  );
...
function runOosWithFrozenParameters(args) {
  ...
  for (let k = 0; k < built.tiles.length; k += 1) {
    ...
    const slicedBm = benchmarkDateIndex.sliceBenchmarkFromIndex(args.benchmarkDateIndex, tile);
    if (!slicedBm.ok) {
      return { ok: false, error: makeError(ERROR.OOS_FOLD_FAILED, { field: "benchmarkSeries" }) };
    }
    foldInput.benchmark = {
      market,
...
function runWalkForwardTrainParameterSelection(input) {
  ...
  for (const window of windowResult.windows) {
    ...
    const oosResult = runOosWithFrozenParameters({
      ...
      benchmarkDateIndex: oosBenchmarkIndex,
      foldId: window.foldId,
      hooks,
    });
