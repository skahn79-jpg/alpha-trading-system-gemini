...
function calculatePerformanceMetrics(input) {
  ...
  if (input.portfolioStatus !== PORTFOLIO_STATUS_COMPLETED) {
    return blockedResult([makeError(ERROR.PORTFOLIO_NOT_COMPLETED, "portfolioStatus")]);
  }
  ...
  if (!isFiniteNumber(initialCapital) || initialCapital <= 0) {
    errors.push(makeError(ERROR.INVALID_INPUT, "initialCapital"));
  }
  if (!isFiniteNumber(finalEquity) || finalEquity < 0) {
    errors.push(makeError(ERROR.INVALID_INPUT, "finalEquity"));
  }

  if (!Array.isArray(input.dailyEquityCurve) || input.dailyEquityCurve.length === 0) {
    errors.push(makeError(ERROR.INVALID_INPUT, "dailyEquityCurve"));
  }
  ...
  if (!Array.isArray(input.closedTrades)) {
    return blockedResult([makeError(ERROR.INVALID_INPUT, "closedTrades")]);
  }
  ...
  for (let i = 0; i < curve.length; i += 1) {
    ...
    if (!isPlainObject(row) || !Object.hasOwn(row, "tradingDate") || !Object.hasOwn(row, "equity")) {
      errors.push(makeError(ERROR.INVALID_EQUITY_CURVE, "dailyEquityCurve"));
      break;
    }
  ...
  if (lastRow.equity !== finalEquity) {
    return blockedResult([makeError(ERROR.INVALID_EQUITY_CURVE, "finalEquity")]);
  }
  ...
  if (!isFiniteNumber(elapsedDays) || elapsedDays < 0) {
    return blockedResult([makeError(ERROR.INVALID_EQUITY_CURVE, "dailyEquityCurve")]);
  }
  ...
  if (!isFiniteNumber(totalReturn)) {
    return blockedResult([makeError(ERROR.INVALID_EQUITY_CURVE, "totalReturn")]);
  }
  ...
  if (!isFiniteNumber(mdd) || mdd > 0) {
    return blockedResult([makeError(ERROR.INVALID_EQUITY_CURVE, "mdd")]);
  }
