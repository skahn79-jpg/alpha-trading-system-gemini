...
function collectUnknownKeys(obj, allowed, fieldPrefix, extra) {
  ...
  for (const key of Object.keys(obj)) {
    if (!allowed.has(key)) {
      // GATE 8R freeze: GATE 8Q spread extra/loc/marketLoc first so code/field win. Do not put extra last.
...
// GATE 9I freeze: execution public result uses ok and status. Do not add failedStage.
...
function validateExecutionCandle(candle, recordIndex) {
  ...
  if (Object.hasOwn(candle, "symbol")) {
    if (typeof candle.symbol !== "string" || !candle.symbol.startsWith("SYNTH")) {
      errors.push(makeSafeExecutionError({ ...loc, code: ERROR.INVALID_SYMBOL, field: "symbol" }));
    }
  }
  ...
  for (const field of priceFields) {
    ...
    if (!isValidPrice(candle[field])) {
      errors.push(makeSafeExecutionError({ ...loc, code: ERROR.INVALID_PRICE, field }));
      pricesOk = false;
    }
  }
  if (
    ...
    if (!(high >= open && high >= close && high >= low && low <= open && low <= close)) {
      errors.push(makeSafeExecutionError({ ...loc, code: ERROR.OHLC_INCONSISTENT, field: "high" }));
    }
  }

  if (Object.hasOwn(candle, "volume") && !isValidVolume(candle.volume)) {
    errors.push(makeSafeExecutionError({ ...loc, code: ERROR.INVALID_VOLUME, field: "volume" }));
  }
  ...
  if (Object.hasOwn(candle, "isFinal") || Object.hasOwn(candle, "candleFinality")) {
    if (candle.isFinal !== true || candle.candleFinality !== CANDLE_FINALITY.FINAL) {
      errors.push(makeSafeExecutionError({ ...loc, code: ERROR.CANDLE_NOT_FINAL, field: "candleFinality" }));
    }
  }
