...
function calculateBenchmarkPerformance(input) {
  ...
  if (!isFiniteNumber(benchmarkReturn) || !isFiniteNumber(alpha)) {
    // 6Z: first-failure-wins field on late INVALID_BENCHMARK_INPUT.
    const field = !isFiniteNumber(benchmarkReturn) ? "benchmarkReturn" : "alpha";
    return blockedResult([makeError(ERROR.INVALID_BENCHMARK_INPUT, { field })]);
  }
