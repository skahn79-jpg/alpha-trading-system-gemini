...
function calculateBenchmarkPerformance(input) {
  ...
  if (input.performanceStatus !== PERFORMANCE_STATUS_COMPLETED) {
    ...
    return blockedResult([makeError(ERROR.PERFORMANCE_NOT_COMPLETED, "performanceStatus")]);
  }

  if (!isFiniteNumber(input.strategyTotalReturn)) {
    return blockedResult([makeError(ERROR.INVALID_BENCHMARK_INPUT, "strategyTotalReturn")]);
  }
  ...
  if (market !== benchmarkMarket) {
    return blockedResult([makeError(ERROR.BENCHMARK_MARKET_MISMATCH, "benchmarkMarket")]);
  }
  ...
  if (!periodStartParsed.ok || !periodEndParsed.ok) {
    return blockedResult([makeError(ERROR.INVALID_INPUT, "period")]);
  }
  ...
  if (periodStartMs == null || periodEndMs == null || !(periodStartMs < periodEndMs)) {
    return blockedResult([makeError(ERROR.BENCHMARK_PERIOD_MISMATCH, "period")]);
  }

  if (!Array.isArray(input.benchmarkSeries) || input.benchmarkSeries.length < 2) {
    return blockedResult([makeError(ERROR.INVALID_BENCHMARK_INPUT, "benchmarkSeries")]);
  }
  ...
  for (let i = 0; i < series.length; i += 1) {
    ...
    if (!isFiniteNumber(row.close) || row.close <= 0) {
      return blockedResult([makeError(ERROR.INVALID_BENCHMARK_INPUT, "benchmarkSeries.close")]);
    }

    const parsed = parseYmd(row.tradingDate);
    if (!parsed.ok || parsed.date == null) {
      return blockedResult([makeError(ERROR.INVALID_BENCHMARK_INPUT, "benchmarkSeries.tradingDate")]);
    }
  ...
  if (!isFiniteNumber(startClose) || !isFiniteNumber(endClose)) {
    return blockedResult([makeError(ERROR.BENCHMARK_PERIOD_MISMATCH, "benchmarkSeries")]);
  }
