...
const ALLOWED_SYNTHETIC_MARKETS = new Set(Object.values(dataValidation.SYNTHETIC_MARKETS));
...
function mapCandleMarketForExecution(market) {
  if (market === dataValidation.SYNTHETIC_MARKETS.SYNTHETIC_KOSPI) {
    return executionModel.MARKET.SYNTHETIC_MARKET;
  }
  return market;
}
...
function mapCandleForExecution(candle) {
  ...
  if (mapped.market != null) {
    mapped.market = mapCandleMarketForExecution(mapped.market);
  }
  return mapped;
}
...
function runSyntheticSingleTradePipeline(input) {
  ...
  if (!costBuilt.ok) {
    const errors = mergeSafeStageErrors(STAGE.COST, costBuilt.errors);
    errors.push(makeSafePipelineError({
