 ...
 * crypto builtin, ./schemas, ./calendar-validation, ./make-error 만 사용한다.
 * 6N: makeError via shared makeBacktestError. Non-object dataset has no extra.
 */
...
const EXACT_CALENDAR_MEMBERSHIP = Symbol("EXACT_CALENDAR_MEMBERSHIP");
...
const VALIDATION_MARKET_CONTRACT = Object.freeze({
  LEGACY_SYNTHETIC: "LEGACY_SYNTHETIC",
  NORMALIZED_SYNTHETIC: "NORMALIZED_SYNTHETIC",
});

const LEGACY_ALLOWED_SYNTHETIC_MARKET_SET = new Set([
  SYNTHETIC_MARKETS.SYNTHETIC_MARKET,
  SYNTHETIC_MARKETS.SYNTHETIC_KOSPI,
  SYNTHETIC_MARKETS.SYNTHETIC_KOSDAQ,
]);

const NORMALIZED_ALLOWED_SYNTHETIC_MARKET_SET = new Set([
  SYNTHETIC_MARKETS.SYNTHETIC_KOSPI,
  SYNTHETIC_MARKETS.SYNTHETIC_KOSDAQ,
]);

const PRODUCTION_MARKET_SET = new Set(["KOSPI", "KOSDAQ"]);
...
const ERROR = Object.freeze({
  ...
  INVALID_MARKET_CONTRACT: "INVALID_MARKET_CONTRACT",
  LEGACY_MARKET_NOT_ALLOWED_IN_NORMALIZED_VALIDATION: "LEGACY_MARKET_NOT_ALLOWED_IN_NORMALIZED_VALIDATION",
...
// GATE 9I freeze: data-validation public result uses ok/status. calendarId is optional-missing. Do not require pipelineStatus.
function createValidationResult(partial) {
  ...
  const errors = Array.isArray(src.errors)
    ...
    : [];
  // 7D: copy warnings so callers cannot mutate the result array.
  const warnings = Array.isArray(src.warnings) ? src.warnings.slice() : [];
...
function runSyntheticCalendarIntegration(dataset, options) {
  ...
  // GATE 8Y freeze: GATE 8X spread identity first so errors/flags win. Do not put identity last.
  ...
  if (calendar[EXACT_CALENDAR_MEMBERSHIP] !== true) {
...
function validateCandle(candle, context) {
  ...
  if (ctx.synthetic === true) {
    ...
    if (PRODUCTION_MARKET_SET.has(candle.market)) {
      errors.push(makeError(ERROR.PRODUCTION_MARKET_NOT_ALLOWED, { ...loc, field: "market" }));
    } else if (ctx.marketContract === VALIDATION_MARKET_CONTRACT.NORMALIZED_SYNTHETIC
      && candle.market === SYNTHETIC_MARKETS.SYNTHETIC_MARKET) {
      errors.push(makeError(ERROR.LEGACY_MARKET_NOT_ALLOWED_IN_NORMALIZED_VALIDATION, {
        ...loc,
        field: "market",
      }));
    } else if (!ctx.allowedSyntheticMarkets.has(candle.market)) {
      errors.push(makeError(ERROR.SYMBOL_MARKET_MISMATCH, { ...loc, field: "market" }));
    }
  }
...
function syntheticMarketRulesForContract(contract) {
  if (contract === VALIDATION_MARKET_CONTRACT.NORMALIZED_SYNTHETIC) {
    return {
      marketContract: contract,
      allowedSet: NORMALIZED_ALLOWED_SYNTHETIC_MARKET_SET,
    };
  }
  return {
    marketContract: VALIDATION_MARKET_CONTRACT.LEGACY_SYNTHETIC,
    allowedSet: LEGACY_ALLOWED_SYNTHETIC_MARKET_SET,
  };
}
...
function validateHistoricalDataset(dataset, opts) {
  ...
  const marketContract = options.marketContract == null
    ? VALIDATION_MARKET_CONTRACT.LEGACY_SYNTHETIC
  ...
  if (!isPlainObject(dataset)) {
    errors.push(makeError(ERROR.INVALID_INPUT));
  ...
  if (marketContract !== VALIDATION_MARKET_CONTRACT.LEGACY_SYNTHETIC
    && marketContract !== VALIDATION_MARKET_CONTRACT.NORMALIZED_SYNTHETIC) {
    // GATE 8T freeze: GATE 8S spread meta first so field wins. Do not put meta last. loc stays first.
    ...
    errors.push(makeError(ERROR.INVALID_MARKET_CONTRACT, { ...meta, field: "marketContract" }));
  }

  for (const key of Object.keys(dataset)) {
    if (!ENVELOPE_KEY_SET.has(key)) {
      errors.push(makeError(ERROR.UNKNOWN_FIELD, { ...meta, field: key }));
    }
  }
  ...
  if (mode === LOAD_MODE.PRODUCTION && syntheticLike) {
    errors.push(makeError(ERROR.SYNTHETIC_FIXTURE_NOT_ALLOWED, { ...meta, field: "sourceType" }));
  ...
  for (const field of REQUIRED_ENVELOPE_FIELDS) {
    if (!Object.hasOwn(dataset, field)) {
      errors.push(makeError(ERROR.MISSING_FIELD, { ...meta, field }));
    }
  }

  if (dataset.canonicalizationVersion !== CANONICALIZATION_VERSION) {
    errors.push(makeError(ERROR.CANONICALIZATION_UNSUPPORTED, { ...meta, field: "canonicalizationVersion" }));
  }

  if (dataset.timezone !== "Asia/Seoul") {
    errors.push(makeError(ERROR.INVALID_INPUT, { ...meta, field: "timezone" }));
  }
  if (dataset.sortOrder !== SORT_ORDER.ASCENDING_BY_TRADING_DATE) {
    errors.push(makeError(ERROR.SORT_ORDER_INVALID, { ...meta, field: "sortOrder" }));
  }
  if (dataset.datasetType !== DATASET_TYPE.HISTORICAL_DAILY_OHLCV) {
    errors.push(makeError(ERROR.INVALID_INPUT, { ...meta, field: "datasetType" }));
  }

  const synthetic = dataset.sourceType === SOURCE_TYPE.SYNTHETIC_FIXTURE;
  if (synthetic) {
    const marketRules = syntheticMarketRulesForContract(marketContract);
    if (typeof dataset.datasetId !== "string" || !dataset.datasetId.startsWith("synthetic-")) {
      errors.push(makeError(ERROR.INVALID_DATASET_ID, { ...meta, field: "datasetId" }));
    }
    ...
    if (dataset.fixtureType !== FIXTURE_TYPE.SYNTHETIC_BACKTEST_DATASET) {
      errors.push(makeError(ERROR.INVALID_INPUT, { ...meta, field: "fixtureType" }));
    }
    if (dataset.verificationStatus !== VERIFICATION_STATUS.TEST_VERIFIED) {
      errors.push(makeError(ERROR.INVALID_INPUT, { ...meta, field: "verificationStatus" }));
    }
    if (dataset.notProductionData !== true) {
      errors.push(makeError(ERROR.INVALID_INPUT, { ...meta, field: "notProductionData" }));
    }
    if (dataset.productionEligible !== false) {
      errors.push(makeError(ERROR.INVALID_INPUT, { ...meta, field: "productionEligible" }));
    }
    if (!Array.isArray(dataset.markets)) {
      errors.push(makeError(ERROR.SYMBOL_MARKET_MISMATCH, { ...meta, field: "markets" }));
    } else {
      for (const market of dataset.markets) {
        if (PRODUCTION_MARKET_SET.has(market)) {
          errors.push(makeError(ERROR.PRODUCTION_MARKET_NOT_ALLOWED, { ...meta, field: "markets", market }));
          break;
        }
        if (marketContract === VALIDATION_MARKET_CONTRACT.NORMALIZED_SYNTHETIC
          && market === SYNTHETIC_MARKETS.SYNTHETIC_MARKET) {
          errors.push(makeError(ERROR.LEGACY_MARKET_NOT_ALLOWED_IN_NORMALIZED_VALIDATION, {
            ...meta,
            field: "markets",
            market,
        ...
        if (!marketRules.allowedSet.has(market)) {
          errors.push(makeError(ERROR.SYMBOL_MARKET_MISMATCH, { ...meta, field: "markets", market }));
          break;
        }
      }
    }
  ...
  if (Array.isArray(dataset.symbols)) {
    if (!isLexSortedUnique(dataset.symbols)) {
      errors.push(makeError(ERROR.SYMBOLS_NOT_SORTED, { ...meta, field: "symbols" }));
    }
  } else if (Object.hasOwn(dataset, "symbols")) {
    errors.push(makeError(ERROR.INVALID_INPUT, { ...meta, field: "symbols" }));
  }

  if (Array.isArray(dataset.markets)) {
    if (!isLexSortedUnique(dataset.markets)) {
      errors.push(makeError(ERROR.MARKETS_NOT_SORTED, { ...meta, field: "markets" }));
    }
  } else if (Object.hasOwn(dataset, "markets")) {
    errors.push(makeError(ERROR.INVALID_INPUT, { ...meta, field: "markets" }));
  }

  const candles = dataset.candles;
  if (!Array.isArray(candles)) {
    if (Object.hasOwn(dataset, "candles")) {
      errors.push(makeError(ERROR.INVALID_INPUT, { ...meta, field: "candles" }));
    }
  } else {
    const seen = new Set();
    ...
    for (let i = 0; i < candles.length; i += 1) {
      ...
      const candleResult = validateCandle(candle, {
        ...
        synthetic,
        marketContract,
        allowedSyntheticMarkets: syntheticMarketRulesForContract(marketContract).allowedSet,
    ...
    if (!candlesSorted) {
      errors.push(makeError(ERROR.CANDLES_NOT_SORTED, { ...meta, field: "candles" }));
    }

    if (adjustmentStatuses.size > 1) {
      errors.push(makeError(ERROR.ADJUSTMENT_POLICY_MISMATCH, { ...meta, field: "adjustmentStatus" }));
    }
    ...
    if (isPlainObject(coverage) && expectedFrom && expectedTo) {
      if (coverage.from !== expectedFrom || coverage.to !== expectedTo) {
        errors.push(makeError(ERROR.COVERAGE_MISMATCH, { ...meta, field: "coverage" }));
      }
    ...
    if (isPlainObject(per)) {
      ...
      errors.push(makeError(ERROR.PER_SYMBOL_COVERAGE_MISMATCH, { ...meta, field: "perSymbolCoverage" }));
    }
  }
