...
function validateHistoricalDataset(dataset, opts) {
  ...
  if (marketContract !== VALIDATION_MARKET_CONTRACT.LEGACY_SYNTHETIC
    ...
    errors.push(makeError(ERROR.INVALID_MARKET_CONTRACT, { field: "marketContract", ...meta }));
  }
    ...
    if (!Array.isArray(dataset.markets)) {
      ...
      for (const market of dataset.markets) {
        if (PRODUCTION_MARKET_SET.has(market)) {
          errors.push(makeError(ERROR.PRODUCTION_MARKET_NOT_ALLOWED, { field: "markets", market, ...meta }));
          break;
        }
        ...
        if (!marketRules.allowedSet.has(market)) {
          errors.push(makeError(ERROR.SYMBOL_MARKET_MISMATCH, { field: "markets", market, ...meta }));
          break;
        }
      }
    }
